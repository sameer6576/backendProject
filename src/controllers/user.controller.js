...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    throw new ApiError(
      500,
      "something went wrong in generateAccessAndRefreshTokens"
    );
  }
};
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  if (!avatar.url) {
    throw new ApiError(400, "Avatar url is missing while uploading");
  }
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  if (!coverImage.url) {
    throw new ApiError(400, "cover image url is missing while uploading");
  }
